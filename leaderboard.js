...
export function simulateCompetitorProgress(competitors) {
  ...
  return competitors.map((c) => {
    ...
    if (Math.random() < 0.15) newStreak += 1;
    return {
      ...c,
      xp: newXP,
      level: newLevel,
...
function getMockCaption(type, idea) {
  const captions = {
    code: [
      `Writing logic for the primary endpoint of '${idea}'.`,
      'Refactored local storage and model functions.',
      'Setting up environment configurations.'
    ],
    figma: [
      `Figma wireframe mapping user flow for '${idea}'.`,
      'Sketching screen layout nodes.',
      'Drafted component layouts.'
    ],
    landing: [
      `Polished landing layout for '${idea}'.`,
      'Added hero copy and value proposition.',
      'Connected responsive CSS breakpoints.'
    ]
  };
